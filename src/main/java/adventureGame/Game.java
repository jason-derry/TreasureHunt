...
public class Game {
	...
	public void checkPosition() {
		...
		if (playerPosition.getX() >= mapSize.getX() || playerPosition.getX() < 0
				|| playerPosition.getY() >= mapSize.getY() || playerPosition.getY() < 0)
		...
		if (playerPosition.getX() == treasure.getX() && playerPosition.getY() == treasure.getY()) {
			...
			for (int i = 1; i < features.size(); i++) {
				if ((playerPosition.getX() == features.get(i).getX()
						&& playerPosition.getY() == features.get(i).getY()))
					feature(i);
			}
			printPosition();
	...
	public Coordinates move(String direction) {
		...
		if (direction.equalsIgnoreCase("north") || direction.equalsIgnoreCase("n")) {
			north();
			System.out.println("\nYou travel " + status.getCurrentDir() + ".");
			checkPosition();
